...
def test_policy_module_actions_mode_does_not_require_code_provider() -> None:
    backend = _CaptureBackend()
    module = PolicyLightningModule(
        backend=backend,
        code_provider=None,
...
def test_policy_module_latent_flow_requires_code_provider() -> None:
    backend = _CaptureBackend()
    module = PolicyLightningModule(
        backend=backend,
        code_provider=None,
...
def test_lam_task_accepts_stage1_batch() -> None:
...
def test_lam_task_transfer_batch_to_device_accepts_stage1_batch() -> None:
    model_config = OmegaConf.create(
        {
            "dim": 128,
            "quant_dim": 8,
...
def test_policy_module_converts_stage2_uint8_video_for_online_lam() -> None:
...
def test_policy_module_resizes_online_lam_video_to_teacher_image_size() -> None:
...
def test_policy_module_validation_step_reads_stage2_batch_metadata() -> None:
    backend = _CaptureBackend()
    module = PolicyLightningModule(
        backend=backend,
        code_provider=_DummyCodeProvider(),
...
def test_policy_module_validation_step_enqueues_payload_for_all_batches() -> None:
    backend = _CaptureBackend()
    module = PolicyLightningModule(
        backend=backend,
        code_provider=_DummyCodeProvider(),
