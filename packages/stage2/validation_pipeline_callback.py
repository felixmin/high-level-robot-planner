...
from stage2.validation_cache import Stage2BucketConfig, Stage2ValidationCache
from stage2.validation_checks import CHECK_TYPES, Stage2ValidationCheck

logger = logging.getLogger("stage2.training")
    ...
    def _create_checks(self, checks_config: dict[str, Any]) -> list[Stage2ValidationCheck]:
        ...
        for instance_name, raw_cfg in checks_config.items():
            ...
            if check_type == "latent_flow_decode" and bool(kwargs.get("enabled", True)):
                ckpt = str(kwargs.get("lam_checkpoint_path", "")).strip()
                if not ckpt:
                    raise ValueError(
                        f"Check {instance_name} ({check_type}) requires non-empty lam_checkpoint_path"
                    )
