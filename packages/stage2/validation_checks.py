...
from stage2.validation_cache import Stage2ValidationCache
...
class LatentFlowDecodeCheck(Stage2ValidationCheck):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.lam_checkpoint_path = str(lam_checkpoint_path or "")
        self.num_samples = int(num_samples)
        self.max_decode_batch_size = int(max_decode_batch_size)
        self._lam_model: Optional[torch.nn.Module] = None
        self._lam_device: Optional[torch.device] = None
        self._load_failed = False

    def _ensure_lam_model(self, device: torch.device) -> Optional[torch.nn.Module]:
        if self._load_failed:
            return None
        if self._lam_model is None:
            ...
            self._lam_model = model
            self._lam_device = torch.device("cpu")

        if self._lam_model is None:
            return None
        if self._lam_device != device:
            self._lam_model = self._lam_model.to(device)
            self._lam_device = device
    ...
    def run(
        ...
        *,
        ...
        lam_model = self._ensure_lam_model(pl_module.device)
        if lam_model is None:
            return self.no_output("lam_model_unavailable")
        if getattr(lam_model, "flow_decoder", None) is None:
            return self.no_output("missing_flow_decoder")
        if getattr(lam_model, "decoder_context_projection", None) is None:
