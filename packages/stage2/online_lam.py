"""
Online LAM label generation helpers.

These utilities adapt temporal frame tensors to the LAM encoder
...
if TYPE_CHECKING:
    from lam import LAMEncoderVQInference


def frames_to_lam_video(frames: torch.Tensor) -> torch.Tensor:
    """
    Convert temporal frame tensors to LAM input layout.

    LAM expects video as [B, 3, T, H, W] float32 in [0, 1].
    """
...
@dataclass
class OnlineLAMConfig:
    lam_checkpoint_path: str


class LAMTaskCodeProvider(torch.nn.Module):
    """Thin adapter over LAMEncoderVQInference exposing code indices for Stage 2."""

    def __init__(self, encoder_vq: LAMEncoderVQInference):
        super().__init__()
    ...
    def train(self, mode: bool = True) -> "LAMTaskCodeProvider":
