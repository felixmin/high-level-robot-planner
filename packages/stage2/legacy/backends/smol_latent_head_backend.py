...
class SmolLatentHeadBackend(torch.nn.Module):
    ...
    - Predicts LAM codes via CE loss from pooled prefix hidden state.
    ...
    def __init__(
        ...
        *,
        ...
        self.lam_head: torch.nn.Linear | None = None
    ...
    def _require_ready(self) -> tuple[torch.device, torch.nn.Module, Any, torch.nn.Linear]:
        ...
        return device, self.vlm, self.processor, self.lam_head
    ...
    def _forward_pooled(self, batch: Stage2Batch) -> torch.Tensor:
    ...
    def _forward_pooled_original(self, batch: Stage2Batch) -> torch.Tensor:
    ...
    def _forward_pooled_optimized(self, batch: Stage2Batch) -> torch.Tensor:
...
class SmolFlowActionBackend(SmolLatentHeadBackend):
    ...
    - Flow-matching head for continuous LAM codebook vectors
