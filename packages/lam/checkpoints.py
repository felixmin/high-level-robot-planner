"""Shared LAM checkpoint loading utilities."""
...
def load_lam_task_from_checkpoint(
    checkpoint_path: str,
    *,
    map_location: Union[str, torch.device] = "cpu",
    strict: bool = True,
    ...
    """
    Load a full LAMTask from a Lightning checkpoint.

    Tries LAMTask.load_from_checkpoint first; falls back to manual
    ...
    try:
        ...
        if "strict" in inspect.signature(LAMTask.load_from_checkpoint).parameters:
            load_kwargs["strict"] = strict
        return LAMTask.load_from_checkpoint(ckpt_path, **load_kwargs)
    except TypeError:
        pass
...
def load_lam_model_weights_only(
...
def load_lam_encoder_vq_inference_from_checkpoint(
    checkpoint_path: str,
    *,
    map_location: Union[str, torch.device] = "cpu",
    strict: bool = True,
    ...
    """
    Load a LAMEncoderVQInference from a checkpoint.
    ...
    if strict and (missing or unexpected):
        raise RuntimeError(
            f"Strict LAM model load failed for {checkpoint_path}. missing={list(missing)} unexpected={list(unexpected)}"
        )
    return LAMEncoderVQInference(model, prune_decoders=prune_decoders)
