...
def select_primary_image_stream(image_streams: Mapping[str, torch.Tensor]) -> torch.Tensor:
...
def temporal_frames_to_bcthw(
    ...
    *,
    ...
    if frames.ndim != 5:
        raise ValueError(f"Expected 5D temporal frames tensor, got {tuple(frames.shape)}")
    ...
    if expected_time_steps is not None and int(video.shape[2]) != int(expected_time_steps):
        raise ValueError(f"Expected T={int(expected_time_steps)}, got T={int(video.shape[2])}")
    return video
...
def move_dataclass_tensors_to_device(batch: Any, device: torch.device) -> Any:
    return type(batch)(**{field.name: move_value_to_device(getattr(batch, field.name), device) for field in fields(batch)})


def uint8_image_streams_to_float32(image_streams: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    return {
        key: value.to(dtype=torch.float32).div_(255.0) if value.dtype == torch.uint8 else value
