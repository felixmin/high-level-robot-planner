...
def _pad_last_dim(array: np.ndarray, *, target_dim: int, fill_value: float) -> tuple[np.ndarray, np.ndarray]:
    if array.ndim != 1:
        raise ValueError(f"Expected 1D stats array, got shape {tuple(array.shape)}")
    if int(array.shape[0]) > int(target_dim):
        raise ValueError(f"Stats dim {int(array.shape[0])} exceeds target_dim {int(target_dim)}")
...
def merge_weighted_stats(
    ...
    for feature_key in feature_keys:
        present = [(stats[feature_key], weights[idx]) for idx, stats in enumerate(stats_by_source) if feature_key in stats]
        if not present:
            continue
        local_weights = np.asarray([w for _, w in present], dtype=np.float64)
        means = [_to_flat_array(entry["mean"], dtype=np.float64) for entry, _ in present]
        ...
        counts = [_to_flat_array(entry.get("count", np.asarray([0], dtype=np.int64)), dtype=np.int64) for entry, _ in present]
        ...
        for mu, sigma, min_value, max_value in zip(means, stds, mins, maxs, strict=True):
            ...
            min_pad, _ = _pad_last_dim(min_value, target_dim=feature_dim, fill_value=np.inf)
            max_pad, _ = _pad_last_dim(max_value, target_dim=feature_dim, fill_value=-np.inf)
        ...
        if np.any(denom <= 0.0):
            raise ValueError(f"Feature {feature_key!r} has no valid stats coverage on some dimensions")
        ...
        for weight_row, mu, sigma in zip(normalized_weight_matrix, mean_arrays, std_arrays, strict=True):
        ...
        if all(int(count.shape[0]) == 1 for count in counts):
            ...
            count_arrays = [
                _pad_last_dim(count.astype(np.int64), target_dim=feature_dim, fill_value=0)[0]
                for count in counts
            ]
        ...
        for quantile_key in quantile_keys:
            if all(quantile_key in entry for entry, _ in present):
                values = [
                    _pad_last_dim(_to_flat_array(entry[quantile_key], dtype=np.float64), target_dim=feature_dim, fill_value=0.0)[0]
                    for entry, _ in present
                ]
                q_value = np.zeros((feature_dim,), dtype=np.float64)
                for weight_row, value in zip(normalized_weight_matrix, values, strict=True):
