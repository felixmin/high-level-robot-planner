...
def infer_batch_size(batch: FoundationBatch) -> int:
    ...
    raise ValueError("Could not infer batch size from FoundationBatch.")
...
def prepare_language_inputs(
    *,
    ...
    if batch.task_text is None:
        raise ValueError("Expected either pretokenized language fields or task_text in FoundationBatch.")
...
def prepare_image_inputs(
    *,
    ...
    if streams is None:
        raise ValueError("Expected image_streams in FoundationBatch.")
    mask_map = batch.image_padding_masks
    if mask_map is None:
        raise ValueError("Expected image_padding_masks in FoundationBatch.")
